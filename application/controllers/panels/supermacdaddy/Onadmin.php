...
class Onadmin extends CI_Controller {
	...
	{	//$this->load->view('ondemand/index');  
		...
		if($this->session->userdata('adminlogin')){
			...
			$data['file']='onadmin/admin_dashboard';			
			
			$this->load->view('onadmin_template',$data);
					
		}else{
			redirect('panels/supermacdaddy/onadmin/login');
		}
	}
	/*------------- LOGIN ----------------*/
	...
	public function login(){ //die('got it');
		if($this->session->userdata('adminlogin')){
			redirect('panels/supermacdaddy/onadmin');
		}else{
			$data['title']  = 'Login';
			$data['file']	= 'onadmin/login';			
			
			$this->load->view('onadmin/login');
			
			$username = $this->input->post('onadmin_email');
			$password = $this->input->post('onadmin_password');
			...
			if($this->input->post('ondemand_btn') == 'Login'){ 			
				...
				$result = $this->Onadmin_model->admin_login($username,$password); //print_r($result); die('yes');
				if(!empty($result)){
					$data = array(
							'id' 			=>	$result['id'],
	...
	public function orders(){
		if($this->session->userdata('adminlogin')){
			$data['title']='Orders';
			$data['file']='onadmin/orders';	
			$driver_id = $this->session->userdata('id');		
			$data['orders']= $this->Onadmin_model->orders($driver_id);
			$this->load->view('onadmin_template',$data);
	...
	public function products(){
		if($this->session->userdata('adminlogin')){
			$data['title']='Products';
			$data['file']='onadmin/products';	
			$user_id = $this->session->userdata('id');		
			$data['products']= $this->Onadmin_model->products($user_id);
			$data['main_categories']= $this->Ondemand_model->main_categories();
			
			if(isset($_POST['deactive'])){ 
				$id = $_POST['deactive']; 
				$product = $this->Onadmin_model->active_product($id); 
				$this->session->set_flashdata('success_msg', 'Product Activated Successfully');
				redirect("panels/supermacdaddy/onadmin/products");
			}
			if(isset($_POST['active'])){ 
			$id = $_POST['active']; 
			$product = $this->Onadmin_model->deactive_product($id); 
		...
		if(isset($_POST['delete_product'])){ 
			 ...
			 $product = $this->Onadmin_model->delete_product($productid); 
			 $this->session->set_flashdata('success_msg', 'Product Deleted Successfully');
			 redirect("panels/supermacdaddy/onadmin/products");
		}
		if(isset($_POST['save_product'])){ 
			$result = $this->Onadmin_model->add_product();
     		if($result==true){
	...
	public function edit_product(){
		$result = $this->Onadmin_model->product_detail();
	...
	public function notification(){
		if($this->session->userdata('adminlogin')){
			...
			$notification = $this->Onadmin_model->notification_history(5); 
	...
	public function notificationcount(){
			$notification = $this->Onadmin_model->notification_history(0); 
			echo count($notification);
	}
	public function tasknotification(){
		if($this->session->userdata('adminlogin')){
			
			$tasknotification = $this->Onadmin_model->tasknotification(5); 
	...
	public function tasknotificationcount(){
			$notification = $this->Onadmin_model->tasknotification(0); 
			echo count($notification);
	}
	public function msgnotification(){
		if($this->session->userdata('adminlogin')){
			 
			$notification = $this->Onadmin_model->msgnotification(5); 
			foreach($notification as $val){
	...
	public function msgnotificationcount(){
			$notification = $this->Onadmin_model->msgnotification(0); 
			echo count($notification);
	}
	public function chat_history(){
		$id = $this->session->userdata('id');
		$history =$this->Onadmin_model->chat_history();
	...
	public function sendmassage(){
		if($this->session->userdata('adminlogin')){
			$data =$this->Onadmin_model->sendmassage();
		 }else{
			redirect("/login");	
		 }
	...
	{
		$data['title']='Drivers';
		$data['file'] = 'onadmin/drivers';
		$data['drivers']= $this->Onadmin_model->drivers();
		$this->load->view('onadmin_template', $data);
	}
